...
MUS_peptable_df = MUS_peptide_df %>% dplyr::select(-Volume)
knitr::kable(MUS_peptable_df)
```

```{r MUS_Peptide_Count_Plot, include=FALSE}
#Create df for peptide count plot
MUS_pepnum_plot_df = MUS_peptide_df%>% group_by(Volume) %>%
...
#Plot noSF protein data similar to peptide bar graph below
MUS_pepnum_plot2_df = MUS_peptide_df %>% group_by(Volume) %>%
...
**Figure X: Peptide counts for murine samples across volume conditions. Error bars represent +/- 1 SD from the mean, calculated across replicates. PG.Quantity (raw) data used.**
...
**Figure X: Boxplot FC distributions of murine dilution series data. PG.Quantity (raw) data used.**
...
```{r PC3_P1_Organize_Data}
#PC3 Gen1 - DIANN raw data processing
diann_PC3Gen1_subset = raw_data_PC3_Gen1 %>%
                      #Filter out peptides with Precursor and Protein Group intensities of 0
                      filter(Precursor.Quantity > 0) %>%
                      filter(PG.Quantity > 0) %>%
                      #Filter our any peptides & protein groups that have a Q value of > 0.01 (FDR 1%)
...
#PC3 XT SpLib - DIANN raw data processing
diann_PC3XTSpLib_subset = raw_data_PC3_XT_SpLib %>%
                      #Filter out peptides with Precursor and Protein Group intensities of 0
                      filter(Precursor.Quantity > 0) %>%
                      filter(PG.Quantity > 0) %>%
                      #Filter our any peptides & protein groups that have a Q value of > 0.01 (FDR 1%)
...
#PC3 Gen1 - Make new Sample, NP, replicate columns in data df with grep/substring
diann_PC3Gen1_subset = diann_PC3Gen1_subset %>% mutate(Sample = sub("[0-9]*_(.*)_NP[0-9].*","\\1",Run),
                                                       NP = sub("[0-9]*_.*_NP(.).*","\\1",Run),
                                                       Replicate = "A")

#PC3 XT SpLib - Make new Sample, NP, replicate columns in data df with grep/substring
diann_PC3XTSpLib_subset = diann_PC3XTSpLib_subset %>% mutate(Sample = sub("^[0-9]*_(.*)([A-Z])_NP.*","\\1",Run),
                                                             NP = sub("[0-9]*_.*_NP(.).*","\\1",Run),
                                                             Replicate = sub("^[0-9]*_.*([A-Z])_NP.*","\\1",Run))

#PC3 Gen1 - Convert output to summarized protein info by removing duplicate peptides
PC3Gen1_proteins = diann_PC3Gen1_subset %>%
                group_by(Run, Protein.Group, PG.Quantity, PG.Normalised, PG.MaxLFQ, PG.Q.Value) %>%
                distinct(Run, Protein.Group, PG.Quantity, PG.Normalised, PG.MaxLFQ, PG.Q.Value, .keep_all = TRUE) %>%
                ungroup() %>%
                mutate(Log2_LFQ_Intensity = ifelse(PG.MaxLFQ == 0, NA, log2(PG.MaxLFQ))) %>%
...
#PC3 XT SpLib - Convert output to summarized protein info by removing duplicate peptides
PC3XTSpLib_proteins = diann_PC3XTSpLib_subset %>%
                group_by(Run, Protein.Group, PG.Quantity, PG.Normalised, PG.MaxLFQ, PG.Q.Value) %>%
                distinct(Run, Protein.Group, PG.Quantity, PG.Normalised, PG.MaxLFQ, PG.Q.Value, .keep_all = TRUE) %>%
                ungroup() %>%
                mutate(Log2_LFQ_Intensity = ifelse(PG.MaxLFQ == 0, NA, log2(PG.MaxLFQ))) %>%
...
#Combine PC3 Gen1 + XT SpLib data
PC3_P1_proteins = rbind(PC3Gen1_proteins,PC3XTSpLib_proteins)

# #PC3 Gen1 - Save protein data table - no sparsity filter
write.table(PC3_P1_proteins, "~/Code/XT_Comparison/20240207_Murine_Liver_Expl_Plasma/Data/SeerXT_PC3P1_proteins.tsv", sep = "\t", row.names = FALSE)
```

```{r PC3_P1_Sparsity_Filter}
#Calculate number of replicates per volume
# num_PC3P1_rep = n_distinct(PC3_P1_proteins$Sample)/n_distinct(PC3_P1_proteins$Volume)
# 
# #50% Sparsity Filter - flat version due to lack of Gen 1 replicates
# seerXT_PC3P1_final_df = PC3_P1_proteins %>%
#   mutate(Feature = paste(NP, Protein.Ids, sep = "_")) %>%
#   group_by(Feature) %>%
#   mutate(percent_detection = n_distinct(Sample)/num_MUS_rep) %>%
#   ungroup() %>%
...
**Table X: Protein counts for PC3 samples in Seer Proteograph Gen 1 v. XT. PG.MaxLFQ (normalized) data used.**

```{r PC3_P1_Num_Table}
#Create df for protein count table
PC3_P1_protnum_df = PC3_P1_proteins %>% dplyr::select(c("Sample","Protein.Ids")) %>%
                                             unique() %>%
                                             dplyr::group_by(Sample) %>%
                                             summarise(Protein_Count = n())

#Create df for peptide count table
PC3_Gen1_peptide_df = diann_PC3Gen1_subset %>% 
                  mutate(Log2_LFQ_Intensity = ifelse(PG.MaxLFQ == 0, NA, log2(PG.MaxLFQ))) %>%
                  filter(!is.na(Log2_LFQ_Intensity))

PC3XTSpLib_peptide_df = diann_PC3XTSpLib_subset %>% 
                  mutate(Log2_LFQ_Intensity = ifelse(PG.MaxLFQ == 0, NA, log2(PG.MaxLFQ))) %>%
                  filter(!is.na(Log2_LFQ_Intensity))

PC3_P1_pepnum_df = rbind(PC3_Gen1_peptide_df,PC3XTSpLib_peptide_df)
PC3_P1_pepnum_df = PC3_P1_pepnum_df %>% dplyr::group_by(Sample) %>%
                                        summarise(Peptide_Count = n())

PC3_P1_counttable_df = PC3_P1_protnum_df
knitr::kable(PC3_P1_counttable_df)
```
...
**Figure X: Protein counts for murine samples across volume conditions. Colors represent different sparsity filters (no filter, 2 replicate minimum, 3 replicate minimum). Error bars represent +/- 1 SD from the mean, calculated across replicates. PG.MaxLFQ (normalized) data used.**

### PC3_XT_SpLib vs PC3_XT_timsLib vs PC3_XT_libfree (all new, search methods 1 vs. 2 vs. 3)

```{r PC3_P2_Load_Data}
#Already loaded Gen1 = raw_data_PC3_Gen1
