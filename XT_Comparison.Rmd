...
```{r PC3_DepthWaterfallPlot}
#Import plasma data from Keshishian et al.
MCP = read.table("~/Code/XT_Comparison/MCP_plasma.txt", sep = "\t", header = TRUE)

#Isolate first Uniprot ID from Protein.Ids
PC3_proteins$SinglePID = gsub(";.*$", "", PC3_proteins$Protein.Ids)

#Combine the data with the Keshishian et. al MCP intensity data
PC3_proteins_MCP = merge(PC3_proteins, MCP, by.x = "SinglePID", by.y = "Uniprot.accession.number", all.x = TRUE)

#Log transform the Normalized.Mean.Intensity from the MCP data
PC3_proteins_MCP$log_MCP_Intensity = log(PC3_proteins_MCP$Normalized.Mean.Intensity, 2)
PC3_proteins_MCP$log_MCP_Intensity[PC3_proteins_MCP$log_MCP_Intensity == "-Inf"] = NA # replace missing with NA

#Keep relevant columns
PC3_proteins_MCP_unique = PC3_proteins_MCP %>% 
  mutate(Feature = paste(NP, Protein.Ids, sep = "_")) %>%
  select(Protein.Group, Genes, NP, Feature, Normalized.Mean.Intensity, Rank, log_MCP_Intensity) %>% 
  unique()

# add in Gene names
# PC3_proteins_MCP_unique = merge(PC3_proteins_MCP_unique, protein_id_annotation %>% select(-Protein.Group), by = "feature", all.x = TRUE)

#Collapse NP information
PC3_proteins_MCP_PG = PC3_proteins_MCP_unique %>% 
  select(Protein.Group, Genes, Normalized.Mean.Intensity, Rank, log_MCP_Intensity) %>% 
  unique()

#Create df for labeling the mean rank of proteins identified
PC3_MCP_stats = PC3_proteins_MCP %>% 
  select(Protein.Group, Rank, log_MCP_Intensity) %>% 
  unique() %>% 
  summarize(mean_rank = mean(Rank, na.rm = TRUE))

PC3_MCP_stats_NP = PC3_proteins_MCP %>% 
  select(Protein.Group, NP, Rank, log_MCP_Intensity) %>% 
  unique() %>% 
  group_by(NP) %>% 
  summarize(mean_rank = mean(Rank, na.rm = TRUE))

#Depth Plot
PC3_depth_NP_plot = PC3_proteins_MCP_unique %>% 
  ggplot(aes(x = Rank, y = log_MCP_Intensity, color = NP)) +
  #geom_point(alpha = 0.2, stat = "unique", size = 2, aes(color = ID, text = Gene.names)) +
  geom_point(alpha = 0.2, stat = "unique", size = 2) +
  #scale_color_manual(values = c(NA, "#22c6c9")) +
  geom_vline(data = PC3_MCP_stats_NP, linetype = 'dashed', aes(xintercept = mean_rank)) +
  geom_text(aes(x = mean_rank,
                y = 0, 
                label = round(mean_rank),
                hjust = -0.2,
                vjust = 1,
                size = 3),
            data = PC3_MCP_stats_NP) +
  theme_bw() + 
  theme(axis.text = element_text(size = 13),
        axis.title = element_text(size = 15, face = "bold"),
        strip.text = element_text(size = 15, face = "bold"),
        legend.position = "none") +
  labs(title = "PC3 Data - Sampling Depth and Coverage",
       y = "log2(Normalized Mean Intensity)") +
  facet_wrap(~NP) #labeller = labeller(NP = np_labs)

PC3_depth_NP_plot

```

**Figure 14: Waterfall plots (Keshishian et al. plasma data comparison) displaying depth of coverage per NP for all PC3 data. The XT NPs (A,B) are similar in MS proteomic coverage to the Gen 1 NPs (1-5).**
...
**Figure 17: Feature CV distributions for PC3 XT search method comparison study, grouped by search method. PG.MaxLFQ (normalized) data used. No obvious change in CVs with change in search method.**
...
**Figure 19: P-value distributions for Wilcox tests performed on the PC3 XT search method comparison data. Log2-transformed data. PG.MaxLFQ (normalized) data used. Wilcox tests are valid.**
...
**Figure 20: Volcano plots showing significantly differentially expressed proteins between PC3 XT samples in search method comparison experiment. Log2-transformed data. PG.MaxLFQ (normalized) data used. No significant differences between any of the search method datasets on the feature level.**
