...
**Table X: Feature counts for murine samples in volume experiment. PG.Quantity (raw) data used.**
...
**Figure X: Feature counts for murine samples across volume conditions. Colors represent different sparsity filters (no filter, 2 replicate minimum, 3 replicate minimum). Error bars represent +/- 1 SD from the mean, calculated across replicates. PG.Quantity (raw) data used.**

\newline

**Table X: Protein counts for murine samples in volume experiment. PG.Quantity (raw) data used.**
...
**Figure X: Protein counts for murine samples across volume conditions. Colors represent different sparsity filters (no filter, 2 replicate minimum, 3 replicate minimum). Error bars represent +/- 1 SD from the mean, calculated across replicates. PG.Quantity (raw) data used.**

\newline

**Table X: Peptide counts for murine samples in volume experiment. PG.Quantity (raw) data used.**
...
MUS_peptide_1Rep_df = MUS_peptide_df  %>%
  mutate(Feature = paste(NP, Protein.Ids, Precursor.Id, sep = "_")) %>%
  group_by(Feature,Volume) %>%
  mutate(detection_repcount = n_distinct(Sample)) %>%
  ungroup() %>%
  filter(detection_repcount >= 1)
...
MUS_peptable_df = MUS_pepnum_df %>% dplyr::select(-Volume)
...
MUS_pepnum_plot = ggplot(MUS_pepnum_plot_df, aes(fill=Replicate, y=Peptide_Count_noSF, x=reorder(as.factor(Volume),Volume))) + 
  ...
  labs(x = "Volume (uL)",
       ...
       title = "Seer Proteograph XT Comparison: Peptide Counts",
       subtitle = "") +
  scale_fill_viridis(discrete = TRUE)
...
**Figure X: Peptide counts for murine samples across volume conditions. Colors represent different replicates (A, B, C) for each volume. No sparsity filter is applied. PG.Quantity (raw) data used.**
...
**Figure X: Peptide counts for murine samples across volume conditions. Colors represent different sparsity filters (no filter, 2 replicate minimum, 3 replicate minimum). Error bars represent +/- 1 SD from the mean, calculated across replicates. PG.Quantity (raw) data used.**
...
**Figure X: Combo box/violin plot for the murine sample volume experiment data showing distributions of mean log2 raw intensities (averaged across 3 replicates per sample) separated on the x axis by sample volume. Number displayed on each boxplot is the mean for that intensity distribution. Letters shown at the top display significance categories determined by ANOVA TukeyHSD test. PG.Quantity (raw) data used.**
...
**Figure X: Feature CV distributions for murine sample volume experiment series, grouped by volume sample. PG.Quantity (raw) data used. No obvious change in CVs with change in sample volume suggests decreasing sample size does not increase technical variability of the Seer Proteograph XT assay.**
...
**Figure X: Boxplot FC distributions of murine sample volume experiment series data. Numbers at the top of each distribution represent median values. PG.Quantity (raw) data used.**
...
**Figure X: P-value distributions for Wilcox tests performed on the mouse dilution series. Log2-transformed data, FC = (low volume - high volume). PG.Quantity (raw) data used.**
...
**Figure X: Volcano plots showing significantly differentially expressed proteins between mouse samples in sample volume experiment. Log2-transformed data, FC = (low volume - high volume). PG.Quantity (raw) data used.**
...
**Table X: Peptide and protein counts for PC3 samples in Seer Proteograph method comparison study (Gen 1, XT SpLib, XT timsLib, XT libfree). PG.MaxLFQ (normalized) data used.**
...
**Figure X: Protein counts for PC3 samples across Proteograph and search method conditions. PG.MaxLFQ (normalized) data used.**
...
**Figure X: Peptide counts for PC3 samples across Proteograph and search method conditions. PG.MaxLFQ (normalized) data used.**
...
PC3_wide_df = PC3XT_feature_noSF_df %>% dplyr::group_by(Feature,Protein.Ids,Method) %>%
                                            summarize(data = list(Log2_LFQ_Intensity), count = n(), .groups = "drop") %>%
                                            pivot_wider(names_from = Method, values_from = c(data,count))
...
**Table X: Feature counts for PC3 XT samples in Seer Proteograph method comparison study (SpLib, timsLib, libfree). PG.MaxLFQ (normalized) data used.**
...
**Figure X: Feature counts for PC3 XT samples across search method conditions. Error bars represent +/- 1 SD from the mean, calculated across replicates. PG.MaxLFQ (normalized) data used.**
...
**Figure X: Combo box/violin plot for the PC3 XT search method comparison showing distributions of mean log2 LFQ intensities (averaged across 2 replicates per sample) separated on the x axis by search method. Number displayed on each boxplot is the mean for that intensity distribution. Letters shown at the top display significance categories determined by ANOVA TukeyHSD test (note: homogeneity of variance and normality assumptions are statistically invalid, so ANOVA is included here only as a visual aid to suggest possible pairwise differences). PG.MaxLFQ (normalized) data used.**
...
**Figure X: Feature CV distributions for PC3 XT search method comparison study, grouped by search method. PG.MaxLFQ (normalized) data used. No obvious change in CVs with change in search method.**
...
**Figure X: Boxplot FC distributions of PC3 XT search method comparison data. Numbers at the top of each distribution represent median values. PG.MaxLFQ (normalized) data used.**
...
**Figure X: P-value distributions for Wilcox tests performed on the PC3 XT search method comparison data. Log2-transformed data. PG.MaxLFQ (normalized) data used.**
...
**Figure X: Volcano plots showing significantly differentially expressed proteins between PC3 XT samples in search method comparison experiment. Log2-transformed data. PG.MaxLFQ (normalized) data used.**
